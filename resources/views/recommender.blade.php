...
@section('title', 'Perfume Recommender')
...
<div class="container mx-auto px-4 py-12">
    ...
    <!-- Form Section -->
    <form action="{{ route('recommender') }}" method="GET" id="form" class="bg-white shadow-2xl rounded-lg p-8 max-w-10xl mx-auto mb-10 mt-10">
            ...
            @foreach($recommendedProducts as $product)
                <div class="bg-white shadow-xl rounded-lg overflow-hidden transform hover:scale-105 transition duration-300">
                    ...
                    @if($product->image_url)
                        <img src="{{ asset('storage/' . $product->image_url) }}" alt="{{ $product->name }}" class="w-full h-48 object-cover transition duration-300 ease-in-out">
                    @else
                    ...
                    <div class="p-4 text-center">
                        ...
                        <p class="text-green-500 text-sm font-medium mb-2">LKR {{ number_format($product->price, 2) }}</p>
                        <p class="text-gray-600 text-sm mb-4">{{ $product->description }}</p>
                        <a href="{{ route('product.detail', $product->id) }}" class="text-indigo-600 hover:underline font-medium">View Details</a>
                    </div>
                </div>
            @endforeach
        </div>
